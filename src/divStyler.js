...
DivStyler.copyStyle = function() {
  ...
  DivStyler.styleCache.outerStyle = DivStyler.NodeOuterStyleNames.map(
    (cssName) => {
      return div.css(cssName);
    }
  );
  DivStyler.styleCache.innerStyle = DivStyler.NodeInnerStyleNames.map(
    (cssName) => {
    ...
    await KFK.updateSelectedDIVs("paste style", async function(div) {
      ...
      if (myNodeType === DivStyler.styleCache.nodetype) {
        for (let i = 0; i < DivStyler.NodeOuterStyleNames.length; i++) {
          if (DivStyler.styleCache.outerStyle[i]) {
            div.css(
              DivStyler.NodeOuterStyleNames[i],
              DivStyler.styleCache.outerStyle[i]
            );
          }
        }
      }
      for (let i = 0; i < DivStyler.NodeInnerStyleNames.length; i++) {
        if (DivStyler.styleCache.innerStyle[i]) {
          innerDiv.css(
            DivStyler.NodeInnerStyleNames[i],
            DivStyler.styleCache.innerStyle[i]
          );
        }
      }
    });
...
DivStyler.vertSizeSmaller = async function(delta) {
  try {
    ...
    if (divNum === 0) {
      ...
      KFK.morphedShape &&
        DivStyler.resizeShape(KFK.morphedShape, "vertSizeSmaller", delta);
    }
  } catch (error) {
    console.error(error);
  }
...
DivStyler.horiSizeBigger = async function(delta) {
  try {
    ...
    if (divNum === 0) {
      ...
      shape && (KFK.morphedShape = shape);
      KFK.morphedShape &&
        DivStyler.resizeShape(KFK.morphedShape, "horiSizeBigger", delta);
    }
  } catch (error) {
    console.error(error);
  }
...
DivStyler.horiSizeSmaller = async function(delta) {
  try {
    ...
    if (divNum === 0) {
      ...
      shape && (KFK.morphedShape = shape);
      KFK.morphedShape &&
        DivStyler.resizeShape(KFK.morphedShape, "horiSizeSmaller", delta);
    }
  } catch (error) {
    console.error(error);
  }
...
DivStyler.vertSizeBigger = async function(delta) {
  try {
    ...
    if (divNum === 0) {
      ...
      shape && (KFK.morphedShape = shape);
      KFK.morphedShape &&
        DivStyler.resizeShape(KFK.morphedShape, "vertSizeBigger", delta);
    }
  } catch (error) {
    console.error(error);
  }
...
DivStyler.snapToGrid = function(jq) {
  ...
  if (tmpLeft % KFK.APP.model.gridWidth < KFK.APP.model.gridWidth * 0.5) {
    newLeft =
      Math.floor(tmpLeft / KFK.APP.model.gridWidth) * KFK.APP.model.gridWidth;
  } else {
    newLeft =
  ...
  if (tmpTop % KFK.APP.model.gridWidth < KFK.APP.model.gridWidth * 0.5) {
    newTop =
      Math.floor(tmpTop / KFK.APP.model.gridWidth) * KFK.APP.model.gridWidth;
  } else {
    newTop =
