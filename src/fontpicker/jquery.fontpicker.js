	...
	$.fn.fontpicker = function(options) {
		...
		var dictionaries = {
			...
			'zh-CN': {
				...
				'select': '选择',
				'styles': '格式',
				'sampleText': '远程办公 在线协作 昆仑博德 在线白板',
				'sampleTextEditable': 'Sample text, editable'
			},	
		};

		var settings = {
			lang: 'zh-CN', // Interface language
		...
		var Fontpicker = (function() {
			...
			Fontpicker.prototype = {
				...
				getFontsList: function() {
					...
					function append(fontType, fontFamily, fontLabel) {
					...
					if (Object.keys(this.options.localFonts).length > 0) {
						...
						for (fontFamily in this.options.localFonts) {
							let fontLabel = this.options.localFonts[fontFamily].label;
							if(!fontLabel) fontLabel = fontFamily;
							append('local', fontFamily, fontLabel);
						}
					}
					...
					if (Object.keys(this.options.googleFonts).length > 0) {
						...
						for (fontFamily in this.options.googleFonts) {
							let fontLabel = this.options.googleFonts[fontFamily].label;
							if(!fontLabel) fontLabel = fontFamily;
							append('google', fontFamily, fontLabel);
						}
					}
